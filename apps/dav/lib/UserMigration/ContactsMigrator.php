...
class ContactsMigrator implements IMigrator {
	...
	private const MIGRATED_PRODID = '-//IDN nextcloud.com//Migrated contact//EN';
	...
	private function getAddressBookExportData(IUser $user, array $addressBookInfo, OutputInterface $output): array {
		...
		foreach ($nodes as $node) {
			if (isset($node[200][$addressBookDataProp])) {
				...
				$vCard->PRODID = ContactsMigrator::MIGRATED_PRODID;
				$vCards[] = $vCard;
			}
		}
	...
	private function importContact(int $addressBookId, VCard $vCard, OutputInterface $output): void {
		$vCard->PRODID = ContactsMigrator::MIGRATED_PRODID;
		...
		try {
			...
			$output->writeln('Error creating contact "' . $vCard->FN . '", skipping…');
		}
	}
	...
	private function importAddressBook(IUser $user, string $initialAddressBookUri, array $metadata, array $vCards, OutputInterface $output): void {
		...
		foreach ($vCards as $vCard) {
			$this->importContact($addressBookId, $vCard, $output);
		}
	}
